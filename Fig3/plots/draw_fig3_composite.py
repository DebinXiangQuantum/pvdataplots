...
def plot_stats(ax: plt.Axes, global_df: pd.DataFrame, panel_label: str) -> None:
    ...
    ax.set_xlim(-0.7, len(x) - 0.25)
    ax.set_ylim(0, 21000)
    ax.set_yticks([0, 5000, 10000, 15000, 20000])
    ax.set_ylabel("PV Capacity (GW)", labelpad=1)
    ax.set_xticks(x)
    ax.set_xticklabels(SCENARIO_SHORT, rotation=55, ha="right", rotation_mode="anchor")
    ...
    ax.text(0.22, 1.035, "Single\nmodel", transform=ax.transAxes, ha="center", va="bottom", fontsize=6, linespacing=0.86, clip_on=False)
    ax.text(0.55, 1.035, "Regional\nleader", transform=ax.transAxes, ha="center", va="bottom", fontsize=6, linespacing=0.86, clip_on=False)
    ax.text(0.82, 1.035, "Cluster\nleader", transform=ax.transAxes, ha="center", va="bottom", fontsize=6, linespacing=0.86, clip_on=False)

    yaxis_transform = ax.get_yaxis_transform()
    ax.text(1.012, existing_capacity, "Existing\ncapacity", transform=yaxis_transform, color=COLORS["target_red"], fontsize=6, va="center", ha="left", linespacing=0.9, clip_on=False)
    ax.text(1.095, irena_2030, "2030", transform=yaxis_transform, color=COLORS["target_blue"], fontsize=6, va="center", ha="right", clip_on=False)
    ax.text(1.095, irena_2050, "2050", transform=yaxis_transform, color=COLORS["target_blue"], fontsize=6, va="center", ha="right", clip_on=False)
    ax.annotate(
        "",
        xy=(1.105, irena_2050),
        xytext=(1.105, irena_2030),
        xycoords=yaxis_transform,
        textcoords=yaxis_transform,
    ...
    iea_x = len(x) - 0.98
    ax.text(iea_x - 0.08, iea_nze_2030, "2030", color=COLORS["target_orange"], fontsize=6, va="center", ha="right", clip_on=True)
    ax.text(iea_x - 0.08, iea_nze_2050, "2050", color=COLORS["target_orange"], fontsize=6, va="center", ha="right", clip_on=True)
    ax.annotate(
        "",
...
def build_figure() -> None:
    ...
    ax_stats = fig.add_axes([right_x0, 0.750, right_x1 - right_x0, 0.170])
    ax_box_utility = fig.add_axes([right_x0, top_y0 - 0.040, right_x1 - right_x0, 0.140])
    ax_box_distributed = fig.add_axes([right_x0, bottom_y0 + 0.10, right_x1 - right_x0, 0.140])

    fig.text(right_x0, top_y1, "c", ha="left", va="top", fontsize=8, fontweight="bold", color=COLORS["text"])
    fig.text(right_x0, top_y0 + 0.085, "d", ha="left", va="top", fontsize=8, fontweight="bold", color=COLORS["text"])
    fig.text(right_x0, bottom_y0 + 0.20, "e", ha="left", va="top", fontsize=8, fontweight="bold", color=COLORS["text"])
    ...
    png_path = OUT_DIR / "Fig3_composite.png"
    fig.savefig(png_path, dpi=600)
    pdf_path = OUT_DIR / "Fig3_composite.pdf"
