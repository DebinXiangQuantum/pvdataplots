...
SCENARIO_SHORT = ["CHN", "USA", "DEU", "EU", "Cont", "Subcont", "Clus5", "Clus10", "Clus20"]
...
def plot_violin_box_inset(
    ...
    axins.set_xlabel("Relative change", fontsize=6, labelpad=0.2)
...
def plot_map_grid(
    ...
    inset_color = COLORS["utility_light"] if kind == "utility" else COLORS["distributed_light"]
    ...
    cax.set_position([pos.x0 + pos.width * 0.46, pos.y0 + pos.height * 0.42, pos.width * 0.34, pos.height * 0.18])
    ...
    cb.set_label("Relative PV capacity change vs actual", fontsize=6, labelpad=0.4)
...
def plot_stats(ax: plt.Axes, global_df: pd.DataFrame, panel_label: str) -> None:
    ...
    ax.barh(y, utility, height=0.64, color=COLORS["utility"], label="Utility-scale PV", zorder=3)
    ax.barh(y, distributed, height=0.64, left=utility, color=COLORS["distributed"], label="Distributed PV", zorder=3)
    ...
    for value, color, linestyle in target_lines:
        ax.axvline(value, color=color, linestyle=linestyle, linewidth=0.75, zorder=1)

    for yi, value, r in zip(y, total, ratio, strict=True):
    ...
    ax.set_xlim(0, 21500)
    ax.set_ylim(len(y) - 0.45, -2.75)
    ax.set_xticks([0, 5000, 10000, 15000, 20000])
    ax.set_xlabel("PV Capacity (GW)", labelpad=1)
    ax.set_yticks(y)
    ax.set_yticklabels(SCENARIO_SHORT)
    ...
    for ypos in [3.5, 5.5]:
        ax.axhline(ypos, color="#CFCBC1", linewidth=0.55, linestyle=":", zorder=1)

    group_transform = ax.get_yaxis_transform()
    group_label_bbox = {"facecolor": "white", "edgecolor": "none", "alpha": 0.78, "pad": 0.55}
    ax.text(0.985, 1.5, "Single-model", transform=group_transform, ha="right", va="center", fontsize=6, color="#4A4A4A", bbox=group_label_bbox)
    ax.text(0.985, 4.5, "Regional-leader", transform=group_transform, ha="right", va="center", fontsize=6, color="#4A4A4A", bbox=group_label_bbox)
    ax.text(0.985, 7.0, "Cluster-leader", transform=group_transform, ha="right", va="center", fontsize=6, color="#4A4A4A", bbox=group_label_bbox)

    target_y_iea = -2.05
    target_y_irena = -0.45
    target_label_bbox = {"facecolor": "white", "edgecolor": "none", "alpha": 0.82, "pad": 0.35}
    ax.text(
        existing_capacity,
        0.985,
        "Existing\ncapacity",
    ...
    ax.annotate(
        "",
        xy=(irena_2050, target_y_irena),
        xytext=(irena_2030, target_y_irena),
        xycoords="data",
        textcoords="data",
    ...
    ax.text(
        ...
        ha="center",
        va="top",
        bbox=target_label_bbox,
        clip_on=True,
    )
    ax.text(irena_2030, target_y_irena - 0.12, "2030", color=COLORS["target_blue"], fontsize=6, ha="center", va="bottom", clip_on=True)
    ax.text(irena_2050, target_y_irena - 0.12, "2050", color=COLORS["target_blue"], fontsize=6, ha="center", va="bottom", clip_on=True)

    ax.annotate(
        "",
        xy=(iea_nze_2050, target_y_iea),
        xytext=(iea_nze_2030, target_y_iea),
        xycoords="data",
        textcoords="data",
    ...
    ax.text(iea_nze_2030, target_y_iea - 0.12, "2030", color=COLORS["target_orange"], fontsize=6, ha="center", va="bottom", clip_on=True)
    ax.text(iea_nze_2050, target_y_iea - 0.12, "2050", color=COLORS["target_orange"], fontsize=6, ha="center", va="bottom", clip_on=True)
...
def plot_change_boxplot(ax: plt.Axes, country_df: pd.DataFrame, kind: str, panel_label: str) -> None:
    ...
    ax.axvline(0, color="#333333", linestyle=(0, (2.2, 1.4)), linewidth=0.65, zorder=1)
    bp = ax.bxp(
        stats,
        positions=positions,
        vert=False,
    ...
    ax.set_yticklabels(SCENARIO_SHORT)
    ax.set_xlabel("" if kind == "utility" else "PV Capacity Change (GW)", labelpad=1)
    ax.grid(axis="x", color="#E5E2DA", linewidth=0.35, zorder=0)
    ax.tick_params(length=2, pad=1)
    ax.spines[["top", "right"]].set_visible(False)
    ax.spines[["left", "bottom"]].set_linewidth(0.5)
...
def add_boxplot_explainer(fig: plt.Figure, bounds: list[float]) -> None:
    ...
    x = 0.47
    half_width = 0.045
    y10, y33, y50, y67, y90, ymean = 0.14, 0.35, 0.52, 0.69, 0.90, 0.44
    line = {"color": "#111111", "linewidth": 0.55, "solid_capstyle": "butt"}
    ...
    ax.plot([x - half_width, x + half_width], [y50, y50], **line)
    ax.scatter([x], [ymean], s=12, facecolor="white", edgecolor="#111111", linewidth=0.55, zorder=3)

    ax.text(0.04, y90, "90th percentile", ha="left", va="center", fontsize=6)
    ax.text(0.04, y50, "Median", ha="left", va="center", fontsize=6)
    ax.text(0.04, y33, "33rd percentile", ha="left", va="center", fontsize=6)
    ax.text(0.58, y67, "67th percentile", ha="left", va="center", fontsize=6)
    ax.text(0.58, ymean, "Mean", ha="left", va="center", fontsize=6)
    ax.text(0.58, y10, "10th percentile", ha="left", va="center", fontsize=6)
...
def build_figure() -> None:
    ...
    left_x0, left_x1 = 0.028, 0.665
    right_x0, right_x1 = 0.725, 0.970
    top_y0, top_y1 = 0.555, 0.975
    bottom_y0, bottom_y1 = 0.115, 0.535

    top_map_spec = fig.add_gridspec(
        1,
        1,
        left=left_x0,
        right=left_x1,
    ...
    bottom_map_spec = fig.add_gridspec(
        1,
        1,
        left=left_x0,
        right=left_x1,
    ...
    print("Drawing utility-scale map grid...")
    plot_map_grid(fig, top_map_spec, world, utility_df, "utility", "a", "Utility-scale PV")
    print("Drawing distributed map grid...")
    plot_map_grid(fig, bottom_map_spec, world, distributed_df, "distributed", "b", "Distributed PV")

    stat_bounds = [right_x0, 0.725, right_x1 - right_x0, 0.235]
    utility_box_bounds = [right_x0, 0.470, right_x1 - right_x0, 0.170]
    box_explainer_bounds = [right_x0 + 0.012, 0.338, right_x1 - right_x0 - 0.024, 0.092]
    distributed_box_bounds = [right_x0, bottom_y0 + 0.035, right_x1 - right_x0, 0.170]
    ...
    panel_x = right_x0 - 0.030
    fig.text(panel_x, stat_bounds[1] + stat_bounds[3], "c", ha="right", va="top", fontsize=8, fontweight="bold", color=COLORS["text"])
    fig.text(panel_x, utility_box_bounds[1] + utility_box_bounds[3] + 0.027, "d", ha="right", va="top", fontsize=8, fontweight="bold", color=COLORS["text"])
    fig.text(panel_x, distributed_box_bounds[1] + distributed_box_bounds[3] + 0.027, "e", ha="right", va="top", fontsize=8, fontweight="bold", color=COLORS["text"])

    plot_stats(ax_stats, global_df, "")
    plot_change_boxplot(ax_box_utility, utility_df, "utility", "")
    add_boxplot_explainer(fig, box_explainer_bounds)
    plot_change_boxplot(ax_box_distributed, distributed_df, "distributed", "")

    png_path = OUT_DIR / "Fig3_composite_v2.png"
    fig.savefig(png_path, dpi=600)
    pdf_path = OUT_DIR / "Fig3_composite_v2.pdf"
