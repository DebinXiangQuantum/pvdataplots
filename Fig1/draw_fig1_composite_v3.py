...
def latitude_aligned_axes_position(
    ...
    xs, ys = project_points([177, 177], [lat_min, lat_max])
    display_points = map_ax.transData.transform(np.column_stack([xs, ys]))
    fig_points = fig.transFigure.inverted().transform(display_points)
...
def plot_longitude_profile(ax: plt.Axes, points: pd.DataFrame) -> None:
    bins = np.arange(-180, 181, 1)
    lon, utility = binned_profile(points, "lon", "utility", bins)
    _, distributed = binned_profile(points, "lon", "distributed", bins)
    util_ymax = nice_upper(utility.max())
    dist_ymax = nice_upper(distributed.max())
    dist_scale = 3
    lower = -dist_ymax * dist_scale * 1.05
    upper = util_ymax * 0.8

    ax.fill_between(lon, 0, utility, color=COLORS["utility"], alpha=0.55, linewidth=0, label="Utility-scale PV")
    ax.plot(lon, utility, color=COLORS["utility"], linewidth=0.8)
    ax.fill_between(lon, 0, -distributed * dist_scale, color=COLORS["distributed"], alpha=0.52, linewidth=0, label="Distributed PV")
    ax.plot(lon, -distributed * dist_scale, color=COLORS["distributed"], linewidth=0.8)
    ax.axhline(0, color="#2b2b2b", linewidth=0.5)
    ax.set_xlim(-180, 180)
    ax.set_ylim(lower, upper)
    yticks = [-dist_ymax * dist_scale, 0, util_ymax / 2, util_ymax]
    ax.set_yticks(yticks)
    ax.set_yticklabels([f"{dist_ymax:g}", "0", f"{util_ymax / 2:g}", f"{util_ymax:g}"])
    ...
    ax.legend(frameon=False, ncol=2, loc="upper right", bbox_to_anchor=(1.0, 1.22), handlelength=1.5)
...
def plot_gdp_quadrant(
    ...
    pos = ax.get_position()
    cax_x = pos.x1 + 0.008
    cax = ax.figure.add_axes([cax_x, pos.y0 + pos.height * 0.18, 0.006, pos.height * 0.64])
...
def main() -> None:
    ...
    # V2 layout: the maps become the main visual field and the Lorenz curves
    # are inset into the lower-left open ocean area of each map.
    ax_map_u = fig.add_axes([0.060, 0.705, 0.805, 0.270])
    ax_map_d = fig.add_axes([0.060, 0.405, 0.805, 0.270])

    ax_total = fig.add_axes([0.895, 0.305, 0.085, 0.070])
    ...
    map_u_pos = ax_map_u.get_position()
    ax_lat_u = fig.add_axes([0.895, map_u_pos.y0+0.01, 0.085, map_u_pos.height*0.9])
    plot_latitude_profile(ax_lat_u, points, "utility", COLORS["utility"], map_ax=ax_map_u)
    ax_lorenz_u = fig.add_axes([0.060, 0.728, 0.150, 0.135])
    ...
    map_d_pos = ax_map_d.get_position()
    ax_lat_d = fig.add_axes([0.895, map_d_pos.y0+0.01, 0.085, map_d_pos.height*0.9], sharey=ax_lat_u)
    plot_latitude_profile(ax_lat_d, points, "distributed", COLORS["distributed"], map_ax=ax_map_d)
    ax_lorenz_d = fig.add_axes([0.060, 0.428, 0.150, 0.135])
    ...
    ax_lon = fig.add_axes(longitude_aligned_axes_position(fig, ax_map_d, 0.305, 0.070))
    plot_longitude_profile(ax_lon, points)
    ...
    label_y_top = 0.988
    label_y_mid = 0.685
    label_y_profile = 0.388
    label_y_country = 0.282
    label_y_gdp = 0.168
    add_panel_label(fig, "a", 0.015, label_y_top)
    add_panel_label(fig, "b", 0.025, 0.895)
    add_panel_label(fig, "c", 0.865, label_y_top)
    add_panel_label(fig, "d", 0.015, label_y_mid)
    add_panel_label(fig, "e", 0.025, 0.595)
    add_panel_label(fig, "f", 0.865, label_y_mid)
    add_panel_label(fig, "g", 0.000, label_y_profile)
    add_panel_label(fig, "h", 0.865, label_y_profile)
    add_panel_label(fig, "i", 0.000, label_y_country)
    add_panel_label(fig, "j", 0.000, label_y_gdp)
    add_panel_label(fig, "k", 0.515, label_y_gdp)

    out_pdf = FIG_DIR / "PDFs" / "Fig1_composite_v3.pdf"
    out_png = FIG_DIR / "Fig1_composite_v3.png"
